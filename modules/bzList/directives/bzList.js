define('modules/bzList/directives/bzList', [
    'angular',
     ...
     */
    app.value('bzListCache', {cacheRes: {}})
        .factory('bzList', ['bzListCache', '$q', function (oiListCache, $q) {

            return function (scope, cache, Resource, generate, options) {

                if (typeof oiListCache.cacheRes[cache] === 'undefined') oiListCache.cacheRes[cache] = {res: [], url: ''};
                var dataRes = oiListCache.cacheRes[cache].res,
                    dataUrl = oiListCache.cacheRes[cache].url, //Требуется, например, для загрузки файлов
                ...
                if (angular.equals(dataRes, [])) {
                    var defer = $q.defer();
                    scope.items = dataRes = oiListCache.cacheRes[cache].res = Resource.query(function (data) {
                        ...
                        if (scope.items === undefined || !scope.items.resName || scope.items.resName === cache) {
                            ...
                            scope.items = dataRes = oiListCache.cacheRes[cache].res
                            defer.resolve(dataRes);
                        }
                    }, function (data) {
                        setError(data.data.error, {msg: 'Невозможно загрузить список. Нет соединения с интернетом', code: 'load'});
                    })
                    defer.promise.then(function (data) {
                        scope.url = oiListCache.cacheRes[cache].url = typeof data !== 'undefined' ? data.config.url : ''
                    });
                ...
                function add(position, afterAdd) {
                    ...
                    Resource.add(function (data) {
                        ...
                        if (typeof afterAdd !== 'function') {
                            ...
                            afterAdd(newIndex, dataRes[newIndex], data);
                        }
