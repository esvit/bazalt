define('components/bcUsers/controllers/Profile/Edit', [
    ...
    app.controller('bcUsers.Controllers.Profile.Edit',
        ['$scope', 'bcUsers.Factories.User', '$rootScope', '$location', '$user', '$routeParams',
            function ($scope, UserResource, $rootScope, $location, $user, $routeParams) {

                $scope.user = UserResource.get({ 'id': $routeParams.user_id }, function(user) {
                    $scope.loading = false;
                    if (!user.images) {
                        //user.images = [];
                    }
                ...
                $scope.saveUser = function(user) {
                    ...
                    $scope.loading = true;
                    user.$save({ 'edit': true }, function(user) {
                        $scope.loading = false;
